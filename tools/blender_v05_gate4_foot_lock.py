...
Usage:
  blender --background --python tools/blender_v05_gate4_foot_lock.py -- \\
...
DEFAULT_OUT = ROOT / "dist" / "v0.5" / "gate4"
DEFAULT_FBX = (
    ROOT
...
def main() -> int:
    ...
    from nurion_v05_body_motion.gate4.foot_lock import run_gate4
    from nurion_v05_body_motion.gate4.parameters import parameter_hash
    ...
    status = {
        ...
        "next": (
            "GATE5_MESH_PENETRATION"
            if result.verdict in ("PASS", "PASS_WITH_LIMITATIONS")
            else "GATE4_FIX"
        ),
    }
    _write(out_dir / "V05_GATE4_STATUS.json", status)
    _write(DEFAULT_OUT / "V05_GATE4_LATEST.json", status)
    _write(
        ROOT / "dist/v0.5/STATUS.json",
        {
            "schema": "NURION_V0.5_STATUS",
            "track": "Body Motion Retarget & Rig Correction",
            "implementation": "GATE4_FOOT_SLIDE_GROUND_LOCK",
            ...
            "gate2ParameterHash": GATE2_HASH,
            "gate3ParameterHash": GATE3_HASH,
            "gate4ParameterHash": parameter_hash(),
            "footSlideInherited": 13,
            "footSlideDisposition": "CONSUMED_IN_GATE4",

"""Pack v0.2.0-alpha.3 ZIP without modifying alpha.1 / alpha.2 / v0.1 artifacts."""
...
ZIP_NAME = "NURION_Character_Landmarker_v0.2.0-alpha.3.zip"
...
def pack() -> Path:
    if ALPHA3_LOCK.exists():
        lock = json.loads(ALPHA3_LOCK.read_text(encoding="utf-8"))
        if lock.get("frozen"):
            raise RuntimeError(
                "alpha.3 is frozen (ALPHA3_LOCK.json). Do not repack; ship a new version path."
            )
    if ALPHA1_ZIP.exists() and sha256_file(ALPHA1_ZIP) != ALPHA1_SHA:
        raise RuntimeError("alpha.1 ZIP hash changed — refuse to pack alpha.3")
    if ALPHA2_ZIP.exists() and sha256_file(ALPHA2_ZIP) != ALPHA2_SHA:
        raise RuntimeError("alpha.2 ZIP hash changed — refuse to pack alpha.3")
    if ALPHA2_LOCK.exists():
        lock = json.loads(ALPHA2_LOCK.read_text(encoding="utf-8"))
        if not lock.get("frozen"):
            raise RuntimeError("alpha.2 lock must remain frozen before packing alpha.3")
    if V01_ZIP.exists() and sha256_file(V01_ZIP) != V01_SHA:
        raise RuntimeError("v0.1 ZIP hash changed — refuse to pack alpha.3")

    init_text = (ADDON_DIR / "__init__.py").read_text(encoding="utf-8")
    if "alpha.3" not in init_text:
        raise RuntimeError("Addon description must mention alpha.3")

    out = OUT_DIR / ZIP_NAME
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    if out.exists():
        if sha256_file(out) == ALPHA3_SHA:
            raise RuntimeError("alpha.3 ZIP is locked — refuse overwrite")
        out.unlink()
    ...
    manifest = {
        ...
        "alpha2Untouched": True,
        "alpha2Sha256": ALPHA2_SHA,
        "v01Untouched": True,
        "v01Sha256": V01_SHA,
    }
    (OUT_DIR / "alpha3").mkdir(parents=True, exist_ok=True)
    (OUT_DIR / "alpha3" / "package-manifest.json").write_text(

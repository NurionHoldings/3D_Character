"""
v0.4 Gate 2 — Procedural Face Guide & Neutral Rig Candidate.

Clone-only. No source mutation. No Viseme/Lip Sync/Full Expression.

Usage:
  blender --background --python tools/blender_v04_gate2_procedural_face_guide.py -- \\
...
DEFAULT_OUT = ROOT / "dist" / "v0.4" / "gate2"
DEFAULT_TENNIS = (
    ROOT
    / "assets"
...
def main() -> int:
    ...
    from nurion_v04_face_rig.gate2.integration import run_gate2
    from nurion_v04_face_rig.gate2.parameters import GATE2_PARAMETERS, parameter_hash
    ...
    status = {
        "schema": "NURION_V04_GATE2_STATUS",
        "gate": "2",
        "name": "PROCEDURAL_FACE_GUIDE_NEUTRAL_RIG_CANDIDATE",
        "V04_GATE2": result.verdict,
        "role": args.role,
        "asset": args.label,
        ...
        "gate1ParameterHashRequired": GATE2_PARAMETERS["gate1ParameterHash"],
        "artifacts": {
            "profile": "GATE2_PROFILE.json",
            "validation": "GATE2_VALIDATION.json",
        },
    }
    _write(out_dir / "V04_GATE2_STATUS.json", status)
    _write(DEFAULT_OUT / "V04_GATE2_LATEST.json", status)
    print(
        json.dumps(
            {
                "V04_GATE2": result.verdict,

#!/usr/bin/env python3
"""Pack NURION-PRODUCT-05_behavior_runtime_proof.zip — READY_FOR_HUMAN_AUDIT only."""
...
ZIP_PATH = EV / "NURION-PRODUCT-05_behavior_runtime_proof.zip"
MANIFEST = EV / "NURION-PRODUCT-05_behavior_runtime_proof_manifest.json"
SIDECAR = EV / "NURION-PRODUCT-05_behavior_runtime_proof.zip.sha256"
...
def main() -> int:
    ...
    subprocess.run(
        [sys.executable, str(ROOT / "tools/build_product05_behavior_runtime.py")],
        check=True,
        cwd=str(ROOT),
        env=env,
    )
    ...
    p04_before = sha256(SEM / "NURION_PRODUCT04_CHARACTER_ENTRANCE_STATE_V1.json")

    proc = subprocess.run(
        [sys.executable, str(ROOT / "tools/run_product05_behavior_runtime_proof.py")],
        capture_output=True,
        text=True,
        cwd=str(ROOT),
        env=env,
    ...
    p04_after = sha256(SEM / "NURION_PRODUCT04_CHARACTER_ENTRANCE_STATE_V1.json")
    if (
        p01_before != p01_after
        or p02_before != p02_after
        or p03_before != p03_after
        or p04_before != p04_after
    ):
        raise SystemExit("BLOCKER: PRODUCT-01/02/03/04 mutated during PRODUCT-05 proof")

    proof = json.loads(
        (EV / "NURION-PRODUCT-05_behavior_runtime_proof_stdout.json").read_text(encoding="utf-8")
    )
    rt_path = SEM / "NURION_PRODUCT05_CHARACTER_INTERACTION_BEHAVIOR_RUNTIME_V1.json"
    runtime = json.loads(rt_path.read_text(encoding="utf-8"))
    rt_canon = hashlib.sha256(
    ...
    digest_path = EV / "NURION-PRODUCT-05_expected_observed_digest.json"
    digest_path.write_text(json.dumps(digest, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")

    critical = {
        g: next((r for r in proof["results"] if r["gate"] == g), None)
        for g in ("P5-G05", "P5-G07", "P5-G08", "P5-G11", "P5-G13", "P5-G14")
    }
    receipt = {
        "receiptId": f"NURION-PRODUCT-05_{ts}",
        "stage": "NURION-PRODUCT-05_CHARACTER_INTERACTION_BEHAVIOR_RUNTIME",
        ...
        "product04": "CLOSED / PASS / CONSUME ONLY",
        "product06": "DENY",
        ...
        "runtimeCanonicalSha256": rt_canon,
        ...
        "upstreamUnmutated": {
            ...
            "PRODUCT-04": p04_before == p04_after,
        },
        "packagedAtUtc": ts,
        "humanAuditRequest": "Independent ZIP reaudit G01–G20; only then CLOSED/PASS",
        "proofStdoutExitCode": proc.returncode,
    }
    receipt_path = EV / "NURION-PRODUCT-05_behavior_runtime_proof_receipt.json"
    ...
    for name in [
        ...
        "NURION-PRODUCT-05_CONTRACT_FREEZE_receipt.json",
        "NURION-PRODUCT-05_PURPOSE_ADOPTION_receipt.json",
        "NURION-PRODUCT-05_behavior_runtime_proof_stdout.json",
    ]:
    ...
    files.extend(
        [
            (
                digest_path,
                "repo/fast_track/working/meshy_silver_starlight/evidence/NURION-PRODUCT-05_expected_observed_digest.json",
            ),
            (
                receipt_path,
                "repo/fast_track/working/meshy_silver_starlight/evidence/NURION-PRODUCT-05_behavior_runtime_proof_receipt.json",
            ),
            (ROOT / "tools/build_product05_behavior_runtime.py", "repo/tools/build_product05_behavior_runtime.py"),
            (
                ROOT / "tools/run_product05_behavior_runtime_proof.py",
                "repo/tools/run_product05_behavior_runtime_proof.py",
            ),
            (ROOT / "tools/run_product05_independent_proof.py", "run_product05_independent_proof.py"),
        ]
    )
    ...
    with zipfile.ZipFile(ZIP_PATH, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        ...
        zf.writestr(
            "evidence/PRODUCT05_proof_runner_console.txt",
            (proc.stdout or "") + "\n--- stderr ---\n" + (proc.stderr or ""),
        )
        zf.writestr(
            "README_AUDIT.json",
            json.dumps(
                {
                    "purpose": "PRODUCT-05 Character Interaction & Behavior Runtime audit",
                    "product05Pass": "NOT_DECLARED",
                    "status": "READY_FOR_HUMAN_AUDIT",
                    "howToIndependentRerun": ["Extract ZIP", "py -3 run_product05_independent_proof.py"],
                    "gates": "P5-G01…G19 automated; P5-G20 human only",
                    "criticalGates": ["P5-G05", "P5-G07", "P5-G08", "P5-G11", "P5-G13", "P5-G14"],
                    "finalLiveIdleAuthority": "PRODUCT-03",
                    "auditFocus": [
                        "Semantic mapping intent→PRODUCT-02 slot",
                        "Single behavior authority dual=false",
                        "Arbitration ACCEPT/QUEUE/REJECT",
                        "Interrupt via INTERRUPTING→RECOVERY (no direct jump)",
                        "Recovery to PRODUCT-03 LIVE_IDLE",
                        "No PRODUCT-01…04 mutation",
                    ],
                },
                indent=2,
                ensure_ascii=False,
    ...
    manifest = {
        "package": "NURION-PRODUCT-05_behavior_runtime_proof.zip",
        "status": "READY_FOR_HUMAN_AUDIT",
        "product05Pass": "NOT_DECLARED",
        "packagedAtUtc": ts,
        ...
        + ["evidence/PRODUCT05_proof_runner_console.txt", "README_AUDIT.json"],
    }
    MANIFEST.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    with zipfile.ZipFile(ZIP_PATH, "a", compression=zipfile.ZIP_DEFLATED) as zf:
        zf.write(MANIFEST, arcname="evidence/NURION-PRODUCT-05_behavior_runtime_proof_manifest.json")
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        ind = subprocess.run(
            [sys.executable, str(td_path / "run_product05_independent_proof.py")],
            capture_output=True,
            text=True,
            cwd=str(td_path),

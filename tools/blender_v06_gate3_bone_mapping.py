"""
v0.6 Gate 3 — unified bone mapping (auto roles, L/R, axes, REST; no manual map).

Usage:
  blender --background --python tools/blender_v06_gate3_bone_mapping.py -- \\
    --zip dist/v0.5/gate1/inbox/ai-aba.bow.zip --label ai-aba.bow --gate2-class LIMITED
"""
...
DEFAULT_OUT = ROOT / "dist" / "v0.6" / "gate3"
...
def main() -> int:
    ...
    from nurion_v06_unified_runtime.gate3.mapping import run_gate3_mapping
    from nurion_v06_unified_runtime.gate3.parameters import GATE3_PARAMETERS, parameter_hash

    if g1_hash() != GATE1_HASH or g2_hash() != GATE2_HASH:
        raise RuntimeError("Gate1/Gate2 parameter hash mismatch — DENY")
    ...
    status = {
        ...
        "manualMappingCount": result.manual_mapping,
        "assetSpecificTuningCount": result.asset_specific_tuning,
        "bodyCoreOk": bool(result.mapping.get("bodyCoreOk")),
        "mappingSufficient": bool(result.validation.get("mappingSufficient")),
        ...
        "artifacts": {
            "profile": "GATE3_PROFILE.json",
            "mapping": "GATE3_MAPPING.json",
            "validation": "GATE3_VALIDATION.json",
        },
        "next": "GATE4_BODY_FACE_EYE_BIND",
    }
    _write(out_dir / "V06_GATE3_ASSET_STATUS.json", status)

"""NURION Parametric Head Candidate A C-Left Half-Open Oral Rig Morph-State Geometry And Projection Parity Diagnostic GO."""
...
COMMAND = (
    "NURION Parametric Head Candidate A C-Left Half-Open Oral Rig Morph-State Geometry "
    "And Projection Parity Diagnostic GO"
)
...
def main() -> int:
    ...
    out = CAND / "runs" / f"v1_lsfq_qa_{run_id}_c_left_oral_geom_diag"
    ...
    probe_path = out / "c_left_oral_rig_morph_state_geometry_probe.json"
    ...
    root = probe.get("rootCauseComparison") or {}
    ...
    receipt = {
        ...
        "runId": run_id,
        "completedAt": now,
        "verdict": "C_LEFT_ORAL_GEOMETRY_DIAGNOSTIC_PASS" if diagnostic_pass else "C_LEFT_ORAL_GEOMETRY_DIAGNOSTIC_OPEN",
        "diagnosticPass": diagnostic_pass,
        "p0Classification": probe.get("p0Classification"),
        "closedBaseline": probe.get("closedBaseline"),
        "cameraProvenance": probe.get("cameraProvenance"),
        "successCriteria": success,
        "rootCauseComparison": root,
        "half": {
            "lower_teeth": (probe.get("half") or {}).get("lower_teeth"),
            "upper_teeth": (probe.get("half") or {}).get("upper_teeth"),
        },
        "open": {
            "lower_teeth": (probe.get("open") or {}).get("lower_teeth"),
            "upper_teeth": (probe.get("open") or {}).get("upper_teeth"),
        },

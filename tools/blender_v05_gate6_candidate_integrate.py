...
Usage:
  blender --background --python tools/blender_v05_gate6_candidate_integrate.py -- \\
...
DEFAULT_OUT = ROOT / "dist" / "v0.5" / "gate6"
DEFAULT_FBX = (
    ROOT
...
def main() -> int:
    ...
    from nurion_v05_body_motion.gate6.integrate import run_gate6
    from nurion_v05_body_motion.gate6.parameters import parameter_hash
    ...
    if _sha(fbx) != FBX_SHA or (zpath.exists() and _sha(zpath) != ZIP_SHA):
        raise RuntimeError("source asset mutated during Gate6 — FAIL")

    _write(out_dir / "GATE6_PROFILE.json", result.profile)
    _write(out_dir / "GATE6_VALIDATION.json", result.validation)

    status = {
        "schema": "NURION_V05_GATE6_STATUS",
        "gate": "6",
        "track": "v0.5 Body Motion Retarget & Rig Correction",
        "name": "CLONE_SAFE_CORRECTION_CANDIDATE",
        "V05_GATE6": result.verdict,
        "label": args.label,
        ...
        "fbxSha256": fbx_sha,
        "candidateAction": (result.profile.get("candidateAction")),
        "v04Sealed": True,
        ...
        "footSlideResidual": (result.profile.get("footSlideFinal")),
        ...
        "artifacts": {"profile": "GATE6_PROFILE.json", "validation": "GATE6_VALIDATION.json"},
        "next": (
            "GATE7_V04_FACE_EYE_LIPSYNC_SYNC_READONLY"
            if result.verdict in ("PASS", "PASS_WITH_LIMITATIONS")
            else "GATE6_FIX"
        ),
    }
    _write(out_dir / "V05_GATE6_STATUS.json", status)
    _write(DEFAULT_OUT / "V05_GATE6_LATEST.json", status)
    _write(
        ROOT / "dist/v0.5/STATUS.json",
        {
            "schema": "NURION_V0.5_STATUS",
            "track": "Body Motion Retarget & Rig Correction",
            "implementation": "GATE6_CLONE_SAFE_CORRECTION_CANDIDATE",
            ...
            "gate4ParameterHash": GATE4_HASH,
            "gate5ParameterHash": GATE5_HASH,
            "gate6ParameterHash": parameter_hash(),
            "candidateAction": result.profile.get("candidateAction"),
            "footSlideResidual": result.profile.get("footSlideFinal"),

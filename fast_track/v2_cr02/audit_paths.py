"""V2-CR-02 audit path resolution — monorepo or extracted self-contained ZIP."""
...
def resolve_v2_cr02_roots(script_file: str | Path) -> dict[str, Path | str | bool]:
    here = Path(script_file).resolve()
    parent = here.parents[1]
    if parent.name == "repo" and (parent / "fast_track" / "v2_cr02").is_dir():
        repo_root = parent
        ...
        mode = "MONOREPO"
        ev = package_root / "evidence"
        rep = package_root / "reports_cr02"
        sem = package_root / "semantic"
        derived = package_root / "derived_cr02"
        idle15 = (
            repo_root

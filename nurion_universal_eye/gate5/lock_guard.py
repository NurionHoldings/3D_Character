"""Refuse Gate5 if Gate1–4B locked sources/params mutated."""
...
def assert_gate1234ab_locked(root: Path) -> dict:
    root = Path(root)
    report = {"ok": True}
    ...
    g2p, g3p, g4ap, g4bp = g2_hash(), g3_hash(), g4a_hash(), g4b_hash()
    report["gate2ParameterHash"] = g2p
    report["gate3ParameterHash"] = g3p
    report["gate4aParameterHash"] = g4ap
    report["gate4bParameterHash"] = g4bp
    if g2p != GATE2_PARAM:
        raise RuntimeError(f"GATE2 PARAMETER MUTATION DENY: {g2p}")
    if g3p != GATE3_PARAM:
        raise RuntimeError(f"GATE3 PARAMETER MUTATION DENY: {g3p}")
    if g4ap != GATE4A_PARAM:
        raise RuntimeError(f"GATE4A PARAMETER MUTATION DENY: {g4ap}")
    if g4bp != GATE4B_PARAM:
        raise RuntimeError(f"GATE4B PARAMETER MUTATION DENY: {g4bp}")
    return report
